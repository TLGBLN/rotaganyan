...
import { parseSireStatBulk } from "../src/lib/sire-stat-parser";

// Kullanım: npx tsx -r dotenv/config scripts/_seed_aygir_generic.ts <dosya> <irk> <mesafe> <pist>
async function main() {
  const [, , dosya, irk, mesafe, pist] = process.argv;
  if (!dosya || !irk || !mesafe || !pist) {
    console.log("Kullanım: _seed_aygir_generic.ts <dosya> <irk> <mesafe> <pist>");
    process.exit(1);
  }
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        await Promise.all(
          chunk.map((p) =>
            db.sireStat.upsert({
              where: { sireStatFiltre: { sireName: p.sireName, ...filtre } },
              create: { ...p, ...filtre },
              update: { ...p },
            })
          )
  ...
  console.log(`${kaydedilen} aygır kaydedildi (${irk}, ${mesafe}, ${pist}).`);
}
