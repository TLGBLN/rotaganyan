...
import { saveDamStatBulk } from "@/server/actions/dam-stat.actions";
...
export default function DamStatForm() {
  ...
  async function handleSave() {
    ...
    setResult(null);
    try {
      const r = await saveDamStatBulk(text, {
    ...
    <div className="space-y-4 rounded-lg border p-4">
      <div>
        ...
        <p className="mt-1 text-[11px] text-muted-foreground">
          hipodromx.com/Kisraklar.aspx sayfasındaki dropdown&apos;larla BİREBİR aynı seçin — aşağıdaki tabloyu bu
          filtrelerle etiketleyip kaydedeceğiz. (Bu site robots.txt ile botları engelliyor, bu yüzden yalnız elle
          yapıştırmayla besleniyor — otomatik çekilmiyor.)
        </p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          placeholder={"Expand\tCOCCINELLE\tCommon Grounds\t1\t3\t3\t%100\t0\t0\t0\t0\t6.372.000\n..."}
          rows={12}
      ...
      {result && (
        <div className="space-y-1 text-xs">
          <p className="text-hit">{result.kaydedilen} kısrak kaydedildi.</p>
