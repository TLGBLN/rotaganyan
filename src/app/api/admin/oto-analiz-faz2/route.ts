...
async function handlePost(req: NextRequest) {
  ...
  const faz1Tablo = faz1.runners
    .map((r) => {
      // TJK'nın resmi at profilinden (AtKosuBilgileri) doğrulanmış kilo/takı değişimi,
      // mevcutsa site DB alanlarından ÖNCELİKLİDİR (kullanıcı talimatı — "Son Yarış
      // Detayları" panelinin kullandığı aynı güvenilir kaynak). Yoksa eskiye geri düşülür.
      const kiloDegisimEfektif = r.sonYarisVeriKaynagiGuvenilir ? r.sonYarisKiloDegisimi : r.weightChange;
      const kiloStr = kiloDegisimEfektif != null ? `${kiloDegisimEfektif >= 0 ? "+" : ""}${kiloDegisimEfektif}kg${r.sonYarisVeriKaynagiGuvenilir ? " [TJK doğrulanmış]" : ""}` : "—";
      const takiEklenenEfektif = r.sonYarisVeriKaynagiGuvenilir ? r.sonYarisTakiEklenen.join(", ") || "—" : r.equipmentAdded ?? "—";
...
Saha kaçak haritası: ${faz1.race.sahadakiKacakSayisi} kaçak → tempo "${faz1.race.kacakTempoEtiketi}" — avantajlı: ${faz1.race.kacakAvantajliStil}${faz1.race.conditions ? `\nŞartlar (TJK): ${faz1.race.conditions}` : ""}${faz1.race.ageWeight ? `\nYaş/Kilo skalası (TJK): ${faz1.race.ageWeight}` : ""}${faz1.race.trackRecord ? `\nPist rekoru (TJK): ${faz1.race.trackRecord}` : ""}${faz1.race.weather ? `\nHava durumu (TJK): ${faz1.race.weather}` : ""}
...
${methodologyText}`;
  ...
  const faz2Tail = `Sen ROTAGANYAN v6.5 at yarışı analistisin. FAZ 2 — PUANLAMA aşamasındasın. Yukarıdaki KOŞU/ATLAR/METODOLOJİ bağlamını kullan (özellikle §VII.0 Sabit 5 Katmanlı Puan Havuzu, ilgili §VII.1-10 kartı, ve §XVIII Tek Puan Sistemi).
...
2. 10+ atlı sahada §VII.0 "Kalabalık Saha Katman Yükseltmesi"ni uygula — Tempo+Stil+Accurace otomatik Katman 1-2'ye yükselir. Ayrıca (§IX.5, v6.4): kalabalık sahada (10+ at) KAÇAK AT stili trafik/blokaj riskiyle dezavantajlı olabilir — bunu olumsuz değil, nötr/dikkatli bir not olarak işle; az atlı sahada (≤6 at) ise SPRINTER/EN GERİ TAKİP tipi atlar (final kapanışa güvenen) daha avantajlıdır, boş alan bulma sorunu yaşamaz — olumlu değerlendir.
...
4d. HAVA DURUMU + KAÇAK AT (§IX.6, v6.4): KOŞU başlığındaki "Hava durumu (TJK)" yağışlı/ıslak bir zemin işaret ediyorsa (kendi muhakemenle değerlendir, sabit anahtar kelime listesi yok), sahadaki KAÇAK AT stilindeki atlar için bunu olumlu bir kombinasyon say — yağışlı/ağır zeminde önden gitmek diğer atların üzerine çamur/su sıçratmaktan kaçınma ve iz bozulmasından önce geçme avantajı sağlar.
4e. ŞARTLI 1/27 TAKISIZ TAY (§XIII.1, v6.4): koşu tipi Şartlı 1 veya Şartlı 27 gibi en giriş seviyeli/genç-ağırlıklı bir koşuysa, TAKISI OLMAYAN (equipment boş/—) taylar takılı olanlara göre KESİNLİKLE olumlu değerlendirilir — bu seviyede takı genelde bir sorunu telafi etmek için takılır, takısız olmak temiz/doğal bir yeteneğe işaret eder.
4f. UZUN ARA + İYİ JOKEY (§XX.29, v6.4): "Son starttan bugüne" 30+ gün ([UZUN ARA] etiketli) ise, galop/kondisyon verisi vasat görünse bile jokeyin kazanma yüzdesi yüksekse bunu olumlu bir kombinasyon say — güçlü jokey uzun aradan dönen atı forma sokabilir, yalnız galop verisine bakarak cezalandırma.
4g. AYGIR/KISRAK AYRI DEĞERLENDİRİLİR (§XII.1, v6.5): "Aygır İstatistiği" (baba) ve "Kısrak İstatistiği" (anne+anne baba) İKİ BAĞIMSIZ sinyaldir, tek bir "pedigri" hükmünde harmanlanıp toplanmaz. Biri (özellikle örneklem küçük/"Kendi verimiz" zayıfsa) zayıf çıksa bile, DİĞERİNİN kendi eşiğini geçen olumlu sinyalini (K/K≥%15 veya AEI>1 gibi) gölgeleme/görmezden gelme — ikisini ayrı ayrı gerekçelendir. Örn: kısrak zayıf ama aygır güçlüyse, notta "pedigri zayıf" denip geçilmez, "aygır tarafı güçlü (K/K %X, AEI Y), kısrak tarafı zayıf" şeklinde ayrıştırılır.
...
6. KİLO-GEÇMİŞ ÇAPRAZ KONTROLÜ: "Aynı Pist/Mesafe/Hipodrom geçmişi" satırındaki geçmiş kilo ("kilo:") bugünküyle karşılaştırılır — geçmişte iyi sonuç aldığı kilodan bugün daha hafifse Katman 4'e +3/+5, geçmişte zaten yetersiz kaldığı kilodan bugün daha ağırsa −4/−6. Aynı satırdaki "derece:" (o günkü bitiriş süresi), "takı:" ve "jokey:" alanları da serbest değerlendirmede kullanılabilir — örn. bugünkü jokey/takı o geçmiş kayıtla aynıysa bu tekrarlanabilirlik lehine ek bir destekleyici not olabilir, "grup:" ise o günkü sınıfı bugünküyle kıyaslamak için bağlam verir.
